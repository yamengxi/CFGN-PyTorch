...
class ContextGuidedModule(nn.Module):
    def __init__(self, in_channels=128, kernel_size=5, dilation=3, reduction=16, act=nn.ReLU, bn=nn.Identity):
        ...
        self.act = act()
        self.f_glo = GlobalContextExtractor(in_channels, reduction)
...
class MainBlock(nn.Module):
    def __init__(self, num_blocks=3, in_channels=128, up_scale=2):
...
class CGSRN(nn.Module):
    def __init__(self, args):
        ...
        self.main_blocks = []
        if args.direct_up:
            self.main_blocks.append(MainBlock(num_blocks=args.n_resblocks, in_channels=args.n_feats, up_scale=1))
            self.main_blocks.append(MainBlock(num_blocks=args.n_resblocks, in_channels=args.n_feats, up_scale=4))
        else:
            for i in range(args.n_resgroups):
                self.main_blocks.append(MainBlock(num_blocks=args.n_resblocks, in_channels=args.n_feats))
        self.main_blocks = nn.Sequential(*self.main_blocks)

        self.HGDModule = HGDModule(args.n_feats, args.n_feats*4, args.n_feats, nn.Identity)
        ...
        if x.shape[-2] % 8 == 0 and x.shape[-1] % 8 == 0:
            ...
            for h_idx in range(h_grids):
                for w_idx in range(w_grids):
                    ...
                    out += F.pad(crop_out, (int(x1)*4, int(w - x2)*4, int(y1)*4, int(h - y2)*4))
